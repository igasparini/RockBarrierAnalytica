...
dt = 1.01e-2 / max(net_nodes_width, net_nodes_height) #8e-3 #1e-2
...
def substep():
    ...
    for n, sid, i in lb_shackle_distances:
        lb_shackle_distances[n, sid, i] = (x_rope[0, i] - x_shackle_hor[n, sid, 0]).norm()
        if lb_shackle_distances[n, sid, i][0] <= 0.1:
