...
@ti.func
def spring_damper_no_compression(x1, x2, v1, v2, spring_const, damper_const, eq_length=-1):
    """
    Models a dashpot spring/damper system that isn't resistant to compression and returns the force.
    ...
    displacement_effective = displacement_norm - eq_length
    spring_force = ti.Vector([0.0, 0.0, 0.0])
    damping_force = ti.Vector([0.0, 0.0, 0.0])
    if displacement_effective > 0:
        spring_force = -spring_const * (displacement_norm - eq_length) * displacement_direction
        damping_force = -damper_const * velocity_difference.dot(displacement_direction) * displacement_direction #* min(net_quad_size_width, net_quad_size_height) # dashpot damping
...
def slide_along_rope_shackles(connections_obj_rope, v_obj, x_obj, v_rope, x_rope, obj_mass, rope_mass, obj_friction_coeff, obj_spring, obj_damper, collision_damper_coeff, dt, num_elements_total):
    ...
    for rid, n, obj_id in connections_obj_rope:
        ...
        shackle_velocity = v_obj[n, obj_id, rid]
        rope_velocity = 0.5 * (v_rope[rid, i_val] + v_rope[rid, i_val+1])
        difference_velocity = shackle_velocity - rope_velocity
        relative_velocity = difference_velocity.dot(current_rope_segment) * current_rope_segment

        remaining_distance = relative_velocity.norm() * dt
        ...
        while remaining_distance > 0:
            ...
            proj_dir_i1 = relative_velocity.dot(dir_i1)
            proj_dir_i2 = relative_velocity.dot(dir_i2)
            proj_dir = 0.0
            ...
            # Friction
            friction_force = -obj_friction_coeff * shackle_velocity
            friction_acceleration = friction_force / obj_mass
            shackle_velocity += friction_acceleration * dt
            ...
            if proj_distance <= segment_length:
                ...
                if overlap > 0:
                    ...
                    v_obj[n, obj_id, rid] = shackle_velocity
    ...
    #     # Node search (10 closest nodes in both directions)
    #     min_distance = (x_obj[n, obj_id, rid] - x_rope[rid, current_i_val]).norm()
    #     best_i_val = current_i_val
    #     for offset in range(-10, 11):
    #         i_val_check = current_i_val + offset
    #         if 0 <= i_val_check < num_elements_ub_total:
    #             distance = (x_obj[n, obj_id, rid] - x_rope[rid, i_val_check]).norm()
    #             if distance < min_distance:
    #                 min_distance = distance
    #                 best_i_val = i_val_check
    #     connections_obj_rope[rid, n, obj_id] = best_i_val
    #     i_val = best_i_val
        
    #     line_point_1 = x_rope[rid, i_val]
    #     line_point_2 = x_rope[rid, i_val+1]

    #     line_direction = (line_point_2 - line_point_1).normalized()

    #     t = (x_obj[n, obj_id, rid] - line_point_1).dot(line_direction)
    #     projected_point = line_point_1 + t * line_direction

    #     # Check if the projection lies on the segment or outside it
    #     if t < 0:
    #         projected_point = line_point_1
    #     elif t > (line_point_2 - line_point_1).norm():
    #         projected_point = line_point_2
    #     else:
    #         projected_point = line_point_1 + t * line_direction

    #     displacement = x_obj[n, obj_id, rid] - projected_point
    #     displacement_direction = displacement.normalized() if displacement.norm() != 0 else ti.Vector([0.0, 0.0, 0.0])
    #     displacement_magnitude = displacement.norm()

    #     rel_velocity = (v_obj[n, obj_id, rid] - v_rope[rid, i_val]).dot(displacement_direction)
    ...
    #     v_obj[n, obj_id, rid] -= total_force * dt / obj_mass
    #     x_obj[n, obj_id, rid] += v_obj[n, obj_id, rid] * dt